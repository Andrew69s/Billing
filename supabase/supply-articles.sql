-- =========================================================
--  СКЛАД · статті списань (2026-09-09)
--  Кожен акт списання має статтю. Вбудовані: store (→ витрати магазину),
--  defect, office, viktor. Кастомні статті адмін додає у kv['supply:articles'].
-- =========================================================

alter table public.supply_acts add column if not exists article text not null default '';

-- наявні списання рахувалися у витрати магазину → лишаємо їх статтею 'store'
update public.supply_acts
   set article = 'store'
 where kind = 'writeoff' and coalesce(article,'') = '';

-- ---------- supply_act: + стаття списання ----------
...
-- ---------- kv: реєстр кастомних статей списань читають усі ----------
